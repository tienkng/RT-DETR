...
from references.deploy.util import remove_high_overlap_bboxes
...
class TRTInference(object):
    ...
    def run_torch(self, blob):
        ...
        for n in self.input_names:
            ...
            assert (
                self.bindings[n].data.dtype == blob[n].dtype
...
if __name__ == "__main__":
    ...
    m.warmup(
        {
            "images": torch.randn(1, 3, 640, 640).to(args.device),
            "orig_target_sizes": torch.tensor([[640, 640]]).to(
                device=args.device, dtype=torch.int32, non_blocking=True
            ),
        },
        10,
    )

    print("Model:", args.trt_file)
    ...
    blob = {
        ...
        "orig_target_sizes": torch.tensor([[w, h]]).to(
            device=args.device, dtype=torch.int32, non_blocking=True
        ),
    }
    start = time.time()
